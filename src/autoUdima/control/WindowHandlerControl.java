...
public class WindowHandlerControl {
	// other constants to avoid magic number
	private static int MAX_DESCRIPTION_LENGTH = 20;



	// tool kit to hava acces to some functionalities of the system
	private Toolkit kit;
	// view part
	...
	public WindowHandlerControl() {
		...
		//Add listeners
		this.addListeners();
		// Set the main window visible after all the process
...
/**
 * Method of windowAdapter, called when a window its closed.
 * (non-Javadoc)
 ...
 */
		...
		/**
		 * Method of windowAdapter called when a window its open
		 * (non-Javadoc)
...
/**
 * Method of windowAdapter, called when a window is in closing state but its not close jet.
 * (non-Javadoc)
 ...
 */
	...
	private class ButtonActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent buttonEvent) {
			...
			if(buttonEvent.getSource() == mainWindow.getAddCarButton() 
					|| buttonEvent.getSource() == mainWindow.getAddCarMenuItem()) {

				createNewCarWindow();

			}

			if(buttonEvent.getSource()== mainWindow.getShowStockButton() 
					|| buttonEvent.getSource() == mainWindow.getShowStockMenuItem()) {

				createStockListWindow();
			}


			if(buttonEvent.getSource()== mainWindow.getRemoveCarButton() 
					|| buttonEvent.getSource()== mainWindow.getRemoveCarMenuItem() ) 
			{
		...
		/**
		 * Method to perform the creation of the registration of a new car in the system
		 */
		...
		/**
		 * private method that add the listener to the new car registration window and components
		 */
		...
		/**
		 * Method that creates the window, that show all the data in the system
		 */
		...
		/**
		 * private method that add the listener to the list window and components
		 */
	...
	private class TextFieldDocumentListener implements DocumentListener{
		...
		/**
		 * Update the document where the event has ocurred
		 * @param event
		 */
	...
	private class FocusTextFieldListener implements FocusListener{
		...
		public void focusGained(FocusEvent eventFocus) {
			...
			/*
			 * Just in case somebody use this foculsListener for other components.
			 * We check if the newCarWindow have been created jet.
			 * 
			 */
			if(newCarWindow != null) {
